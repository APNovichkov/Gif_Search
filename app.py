...
def index():
    ...
    # get the top trending GIFs - using the default locale of en_US
    top_10_gifs = requests.get("https://api.tenor.com/v1/trending?key=%s&limit=%s" % (apikey, lmt))

    # get the top random GIFs - using the default locale of US 
    random = requests.get("https://api.tenor.com/v1/random?key=%s&limit=%s" % (apikey, lmt))
    ...
    if top_10 == "true" or tenor_request.status_code != 200 or len(tenor_request.content) == 0:
        ...
        loaded_gifs = json.loads(random.content)
    else:
